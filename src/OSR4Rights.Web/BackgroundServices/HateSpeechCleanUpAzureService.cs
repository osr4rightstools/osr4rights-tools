...
{
    public class HateSpeechCleanUpAzureService : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            try
            {
                ...
                Log.Information($"Started 2x.{nameof(HateSpeechCleanUpAzureService)}");
                ...
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Log.Error(ex,
                            $"{nameof(HateSpeechCleanUpAzureService)} Outer exception handler - something threw in the {nameof(DoWork)} method's catch or finally. We want the loop to keep going, so it will try again after {timeBetweenChecks.TotalMinutes} minutes");
            ...
            {
                Log.Warning($"{nameof(HateSpeechCleanUpAzureService)} Global - Operation cancelled - can happen when app is shutting down gracefully");
            }
            catch (Exception ex)
            {
                Log.Fatal(ex, $"{nameof(HateSpeechCleanUpAzureService)} Global - Stopped fatally! The service is not running now.");
            }
        }
        ...
        private static async Task DoWork(CancellationToken stoppingToken)
        {
            ...
            //if (!rgs.Any()) Log.Information("No HateSpeech rg's found starting with: webhatespeechcpu");
                ...
                if (vmIdParseResult)
                {
                    ...
                    if (mostRecentJobOnVm is { })
                    {
                        ...
                        if (mostRecentLogDateTimeUtc is { })
                        {
                            ...
                            {
                                Log.Information($"HS Rule 1 Found rg {rg.Name} leaving as time since last log entry is {timeSinceLastLogEntry.TotalMinutes:F1} minutes and we have a window of {timeToKeepVmSinceLastLogEntry.TotalMinutes} minutes");
                            ...
                            else
                            {
                                ...
                                Log.Information($"HS Rule 1 Deleting rg {rg.Name} as time since last log entry is {timeSinceLastLogEntry.TotalMinutes:F1} minutes which is greater than {timeToKeepVmSinceLastLogEntry.TotalMinutes} minute window");

                                // it's possible the job is taking longer than window period 
                                if (mostRecentJobOnVm?.JobStatusId == Db.JobStatusId.Running)
                                {
                                    Log.Information($"HS Rule 1 - Deleting rg {rg.Name} and updating Job Status from Running to Exception as no log messages for time window {timeToKeepVmSinceLastLogEntry.TotalMinutes:F1} minutes");
                            ...
                            Log.Information($"HS Rule 1 Deleting {rg.Name} as no log entries which is unusual");
                        ...
                        Log.Error($"HS can't get most recent job on VM from Db with vmId {vmId}, so deleting the rg");
                }
                else
                    Log.Error($"HS can't parse vmId from Azure - {vmIdString} - PROBLEM! Deleting rg anyway");

                // if either rules say we should delete, then delete
                //if (shouldDeleteVM || shouldDeleteVMB)
                if (shouldDeleteVM)
                {
                    Log.Information($"HS Deleting Rg: {rg.Name}");
                    try
                    {
                    ...
                    {
                        Log.Warning(ex, $"{nameof(HateSpeechCleanUpAzureService)} and method {nameof(DoWork)} catch - threw an Exception while deleting rg {rg.Name}. This can happen when the Rg has been deleted already. Continuing as if it had worked");
                        // swallow exception and continue on
                    }

                    Log.Information($"{nameof(HateSpeechCleanUpAzureService)} Update vmId {vmId} in our database to Deleted, and DateTimeUtcDeleted");
...
// ssh -o StrictHostKeyChecking=no dave@webhatespeechcpu75.cloudapp.azure.com
