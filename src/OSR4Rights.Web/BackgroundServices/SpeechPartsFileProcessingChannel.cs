...
{
    public class SpeechPartsFileProcessingChannel
    {
        private const int MaxMessagesInChannel = 100;

        private readonly Channel<string> _channel;

        public SpeechPartsFileProcessingChannel()
        {
        ...
        public async Task<bool> AddFileAsync(string fileName, CancellationToken ct = default)
        {
            while (await _channel.Writer.WaitToWriteAsync(ct) && !ct.IsCancellationRequested)
            {
                ...
                {
                    Log.Information($"{nameof(SpeechPartsFileProcessingChannel)} added {fileName}");
                    return true;
                }
            }
        ...
        public int CountOfSpeechPartsProcessingChannel() => _channel.Reader.Count;
